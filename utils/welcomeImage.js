...
GlobalFonts.registerFromPath(require.resolve('@fontsource/jost/files/jost-latin-700-normal.woff2'), 'JostBold');
GlobalFonts.registerFromPath(require.resolve('@fontsource/jost/files/jost-latin-600-normal.woff2'), 'JostSemiBold');
...
function getDominantColor(img, size = 64) {
    ...
    if (!best) return { r: 212, g: 168, b: 83 };
...
async function generateWelcomeBanner(member) {
    ...
    const dom       = getDominantColor(avatar);
    const glow      = rgbToHex(dom);
    ...
    const ySub       = yLine + 24;
    const yEldoria   = ySub + 46;

    ctx.font         = '72px JostBold';
    ctx.textAlign    = 'left';
    ctx.textBaseline = 'alphabetic';

    // Mesure du v pour masquer sa pointe
    ...
    ctx.fillText(member.guild.name, TX, yEldoria);
